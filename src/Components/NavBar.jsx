...
const NavBar = () => {
  ...
  useEffect(() => {
    setNavBar();
    window.addEventListener("wheel", setNavBar);
    return () => {
      window.removeEventListener("wheel", setNavBar);
    };
  }, []);
  return (
    <div ref={navbar} className="fixed z-[999] font-['Neue_Montreal'] h-[65px] w-full py-2 px-[4vw] flex items-center justify-between transition-all duration-700  backdrop-blur ">
      <div className="logo h-full w-fit pt-2">
        <svg
          width="72"
          height="30"
          viewBox="0 0 72 30"
      ...
      <div className="links hidden h-full w-full md:flex justify-end gap-7 items-center ">
        {["Services", "Our work", "About us", "Insights", "Contact us"].map(
          (item, idx) =>
            idx == 4 ? (
              <a
                key={idx}
                className="text-[1.1em] font-thin ml-[20%] "
                href="#"
              >
                {item}
              </a>
            ) : (
              <a key={idx} className="text-[1.1em] font-thin " href="#">
                {item}
              </a>
            )
        )}
      ...
      <div className="menuBar md:hidden">
        <i className="ri-menu-3-line font-semibold text-2xl"></i>
      </div>
    </div>
  );
};
